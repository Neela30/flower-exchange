...
void testScenarioExample4() {
    ...
    const std::vector<flower_exchange::test_utils::ComparableReportRow> expected{
        ...
        flower_exchange::test_utils::expectedRow("ord3", "aa15", "Rose", 1, "Pfill", 100, "45.00", ""),
        flower_exchange::test_utils::expectedRow("ord2", "aa14", "Rose", 2, "Fill", 100, "45.00", ""),
    };

    const auto actual = flower_exchange::test_utils::runScenarioThroughReaderAndExchange(
        inputRows, "scenario_example4.csv");
    flower_exchange::test_utils::compareReportsExact(actual, expected, "Scenario Example 4");
}
...
int main() {
    try {
        testScenarioExample4();
    } catch (const std::exception& exception) {
        std::cerr << "ScenarioExample4Tests failure: " << exception.what() << '\n';
        return 1;
    }
