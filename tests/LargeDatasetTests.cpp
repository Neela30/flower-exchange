...
void testInvalidMixSubsetParsingBehavior() {
    CsvOrderReader reader;
    ...
    expect(parsedRows > 0, "Invalid-mix subset should include valid parseable rows");
    expect(failedRows > 0, "Invalid-mix subset should include parser-rejected rows");
}
...
int main() {
    try {
        ...
        testInvalidMixSubsetParsingBehavior();
