...
void testScenarioExample3() {
    ...
    const std::vector<flower_exchange::test_utils::ComparableReportRow> expected{
        ...
        flower_exchange::test_utils::expectedRow("ord3", "aa15", "Rose", 1, "Fill", 100, "45.00", ""),
    ...
    const auto actual = flower_exchange::test_utils::runScenarioThroughReaderAndExchange(
        inputRows, "scenario_example3.csv");
    flower_exchange::test_utils::compareReportsExact(actual, expected, "Scenario Example 3");
}
...
int main() {
    try {
        testScenarioExample3();
    } catch (const std::exception& exception) {
        std::cerr << "ScenarioExample3Tests failure: " << exception.what() << '\n';
        return 1;
    }
