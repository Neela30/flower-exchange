...
namespace
{
    ...
    int countByStatus(const std::vector<ExecutionReport> &reports, ExecStatus status)
    {
        return static_cast<int>(std::count_if(
            reports.begin(),
            reports.end(),
            [status](const ExecutionReport &report)
    ...
    void testScenarioEValidationFailuresProduceRejectedReports()
    {
        ...
        expect(parsedOrderCount == 4, "Scenario E should parse all invalid rows and let validation reject them");
        expect(rejectedCount == 4, "Scenario E should produce four Rejected reports with reasons");
        expect(parseFailureCount == 0, "Scenario E should not fail during CSV parsing");
    }
